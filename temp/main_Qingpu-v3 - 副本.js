...
async function init() {
    ...
    function onDocumentMouseMove( event ) {
        ...
        if (parent.app.switch_interact) checkIntersection();
    }
...
function animate() {
    ...
    function render() {
        ...
        renderer.clear();
        if (parent.app.switch_interact) {
            composer.render();
            // scene_lut rendering removed
        }
        else {
        ...
        function updateModels(renderNum, renderList) {
            if (animationEnabled) {
                ...
                updateExtrudedModelsAnimated(
                    ...
                    (window.parent && window.parent.app) ? window.parent.app : {},
...
async function setBuildingData() {
    ...
    try {
        ...
        // Commented out Supabase query since it's not properly initialized
        ...
        // Use mock data instead since Supabase is not available
        const mockData = {
        ...
        // Update structure attributes if they exist
        ...
        if (structAttrs) {
            ...
            if (structAttrs.bid) structAttrs.bid.value = selectedObject.userData.parent.ib;
            if (structAttrs.height) structAttrs.height.value = selectedObject.userData.parent.height;
            if (structAttrs.floor) structAttrs.floor.value = selectedObject.userData.parent.floor;
            if (structAttrs.coordinate) structAttrs.coordinate.value = "(" +
                geo.boundingSphere.center.x.toFixed(1).toString() + ", " +
                geo.boundingSphere.center.z.toFixed(1).toString() + ")";
            if (structAttrs.boundSp) structAttrs.boundSp.value = geo.boundingSphere.radius.toFixed(2);
            if (structAttrs.triangles) structAttrs.triangles.value = geo.attributes.position.count;
        }
    } catch (error) {
