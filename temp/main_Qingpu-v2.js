...
import { mergeGeometries } from './modules/three-r175/examples/jsm/utils/BufferGeometryUtils.js';
// ------------------------------------------------------custom---------------------------------------------------------
...
import { BlockBuildingChunk }            from './modules/blockBuildingChunk.js';
...
async function loadChunkFilesList() {
    try {
        const response = await fetch('./data/Qingpu/chunkFiles-Qingpu-20250107.json');
        if (!response.ok) {
    ...
    async function initData() {
        ...
        try {
            ...
            const [metaData, mapData, dcjData, eqHisData] = await Promise.all([
                fetchJSON('./data/Qingpu/', 'meta-Qingpu-20250107.json'),
...
function createBuildingsFromChunk(chunkData) {
    // let group = new THREE.Group();
    let geometries = [];
    // console.log(`Processing chunk: ${chunkData.name} with ${chunkData.buildings.number} buildings`);

    chunkData.buildings.data.forEach(building => {
        // Ensure building has necessary properties
        let {
            id,
            bounds,
            center,
        ...
        let block = new BlockBuildingChunk({
            buildings: {
                bounds: bounds,
                height: storey * 3.6, // Assuming 3.5m per floor
            },
            coordinate_scale: chunkData.coordinate_scale,
        }, id, 3.5);

        let mesh = block.getMesh(1); // Assuming '1' is for extruded model
        mesh.userData = {
            id,
            ...building // Store additional metadata if needed
        };
    ...
    // Merge all geometries in the chunk
    ...
    mergedMesh.userData = { chunkIndex: chunkData.chunk_index };
