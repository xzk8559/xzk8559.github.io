...
export function quadTree(map, buildings, iteration) {
    ...
    let offset = 25;
    let x_0 = handle_x( map.mapbounds["maxlon"] ) * map['coordinate scale'];
    let x_1 = handle_x( map.mapbounds["minlon"] ) * map['coordinate scale'];
    let z_0 = handle_y( map.mapbounds["maxlat"] ) * map['coordinate scale'];
    let z_1 = handle_y( map.mapbounds["minlat"] ) * map['coordinate scale'];
    let x_max = offset;
    let x_min = -(Math.max( x_0, x_1 ) - Math.min( x_0, x_1 )) - offset;
    let z_max = (Math.max( z_0, z_1 ) - Math.min( z_0, z_1 )) + offset;
    let z_min = -offset;
